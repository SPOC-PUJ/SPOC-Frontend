...
import { ReaderMatService } from '@/services/ReaderMatService'; // Importar el servicio para archivos MAT
  ...
  if (file.name.toLowerCase().endsWith(".edf")) {
    ...
    reader.onload = async () => {
      try {
        ...
        const processedData = response.data; // Ajustar según la estructura real

        // Emitir los valores procesados
        emit('fileProcessed', processedData); // Ajustar según tus necesidades
    ...
    reader.onload = async () => {
      try {
        ...
        // Enviar el Uint8Array al servicio correspondiente
        const response = await ReaderMatService.ReadMat(fileBytes, "m_Trials_selected");
        console.log('Respuesta del servidor:', response);
