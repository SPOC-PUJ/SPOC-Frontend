<template>
    <div>
      <h1>Subir archivo MAT</h1>
      <input type="file" @change="onFileChange" accept=".mat" />
      <button @click="uploadFile">Subir</button>
    </div>
  </template>
  
  <script>
  import { ReaderService } from '@/services/ReaderMatService';
        ...
        if (this.file) {
          ...
          reader.onload = async () => {
            try {
              ...
              const response = await ReaderService.ReadMat(fileBytes,"m_Trials_selected");
